...
contract CESSPOOL is
    Context,
    IBEP20,
    Ownable //change contract name
{
    ...
    address payable public _cessRewardAddress =
        payable(0x72afA21b2CC9C9a46e712fa171E070dF39a81302); // TO-DO Reward Wallet (CESS)
    address payable public _demonomicsAddress =
        payable(0x07988f3689f793c1784564854c064E02f886D378); // TO-DO Demonomics (BNB)
    address payable public _bnbRewardAddress =
        payable(0x164078Ec49725a6FBDd1d29eEbE3507818fc5276); // TO-DO Top demons (BNB)
    address payable public _devAddress =
        payable(0x55dc94c84d5B691F2442aeD5e5d9aDa386b07dBA); // TO-DO Dev Address (BNB)
    ...
    uint256 private _totalBNBSendFee =
        _liquidityFee.add(_demonomicsFee).add(_devFee).add(_bnbRewardFee); // liquidity + dev + BNB Reward + demonomics fee on each transaction
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(
            !_isExcludedFromReflection[sender],
    ...
    function excludeFromReward(address account) public onlyOwner {
        ...
        require(
            ...
            "Account is already excluded from reflection"
        );
    ...
    function includeInReward(address account) external onlyOwner {
        require(
            _isExcludedFromReflection[account],
            "Account is already included in reflection"
        );
     ...
     */
    function _sendToCESSRewardAddress(address account, uint256 amount)
        internal
    {
        if (amount > 0) {
            uint256 currentRate = _getRate();
            uint256 rAmount = amount.mul(currentRate);
            _rOwned[_cessRewardAddress] = _rOwned[_cessRewardAddress].add(
                rAmount
            );
            if (_isExcludedFromReflection[_cessRewardAddress]) {
                _tOwned[_cessRewardAddress] = _tOwned[_cessRewardAddress].add(
                    amount
                );
            }
    ...
    function setdevFeePercent(uint256 Fee) external onlyOwner {
        _devFee = Fee;
        _totalBNBSendFee = _liquidityFee.add(_demonomicsFee).add(_devFee).add(
            _bnbRewardFee
        );
    }
     ...
     */
    function calculateCESSRewardFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_cessRewardFee).div(10**3);
    }
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_totalBNBSendFee).div(10**3);
    }
    ...
    function removeAllFee() private {
        if (_totalBNBSendFee == 0 && _cessRewardFee == 0 && _reflectionFee == 0)
            return;
    ...
    function _transfer(
        ...
        require(
            _isExcludedFromAntiWhale[to] ||
                balanceOf(to) + amount <= _maxTokensPerAddress,
            "Max tokens limit for this account exceeded. Or try lower amount"
        );
        require(
            _isExcludedFromTransactionCoolDown[from] ||
                block.timestamp >=
                _transactionCheckpoint[from] + _transactionCoolTime,
            "Wait for transaction cooldown time to end before making a tansaction"
        );
        require(
            _isExcludedFromTransactionCoolDown[to] ||
                block.timestamp >=
                _transactionCheckpoint[to] + _transactionCoolTime,
            "Wait for transaction cooldown time to end before making a tansaction"
        );
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 swapPercent = _devFee
            .add(_demonomicsFee)
            .add(_bnbRewardFee)
            .add(_liquidityFee / 2);
        uint256 swapTokens = tokenBalance.div(_totalBNBSendFee).mul(
            swapPercent
        );
    ...
    function _tokenTransfer(
        ...
        if (
            _isExcludedFromReflection[sender] &&
            !_isExcludedFromReflection[recipient]
        ) {
     ...
     */
    function blacklistMultipleWallets(address[] calldata accounts)
        public
        onlyOwner
    {
        require(
            accounts.length < 800,
            "Can not blacklist more then 800 address in one transaction"
        );
     ...
     */
    function unBlacklistMultipleWallets(address[] calldata accounts)
        public
        onlyOwner
    {
        require(
            accounts.length < 800,
            "Can not Unblacklist more then 800 address in one transaction"
        );
        for (uint256 i; i < accounts.length; ++i) {
    ...
    function recoverTokens(
        ...
        require(
            balance >= amountToRecover,
            "Not Enough Tokens in contract to recover"
        );
